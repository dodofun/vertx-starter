...
@Singleton
public final class DemoRedis implements RedisService<Long, Long, Demo> {
    private static final Logger LOGGER = LoggerFactory.getLogger(DemoRedis.class);
    // 过期时间设置
    ...
    @Inject
    public DemoRedis(final RedisClient client) {
    ...
    private static final class KeyUtil {
        ...
        private static final String ENTITY_PREFIX = "demo:";
    ...
    @Override
    public void add(final Long id, final Demo obj) {
    ...
    @Override
    public void addObj(final Long id, final Demo obj) {
    ...
    @Override
    public void addZset(final Long id, final Demo obj) {
    ...
    @Override
    public void addMap(final Long id, final Map<Long, Demo> map) {
        map.forEach((key, value) -> add(id, value));
    }

    @Override
    public void addList(final Long id, final List<Demo> list) {
        list.forEach(value -> {
            addObj(id, value);
            addZset(id, value);
        });
    ...
    @Override
    public Optional<Demo> get(final Long id, final Long id2) {
        try {
            final byte[] bytes = commands.get(KeyUtil.entityKey(id, id2)).block();
            if (Objects.nonNull(bytes) && (bytes.length > 0)) {
                return Optional.of(Demo.parseFrom(bytes));
            }
        } catch (final Exception e) {
    ...
    @Override
    public void update(final Long id, final Long key, final Demo obj) {
        add(id, obj);
    }
